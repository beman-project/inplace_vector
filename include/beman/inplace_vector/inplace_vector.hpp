...
concept container_compatible_range_impl = requires(T&& t) {
    { std::ranges::begin(t) } -> std::same_as<typename std::remove_reference_t<T>::iterator>;
    { std::ranges::end(t) } -> std::same_as<typename std::remove_reference_t<T>::iterator>;
};
} // namespace detail
template <typename T>
...
struct inplace_vector_destruct_base {
    ...
    inplace_vector_destruct_base(const inplace_vector_destruct_base& other) noexcept(
    ...
    operator=(const inplace_vector_destruct_base& other) noexcept(std::is_nothrow_copy_constructible_v<T> &&
                                                                  std::is_nothrow_copy_assignable_v<T>) {
        size_ = other.size_;
    }

    inplace_vector_destruct_base&
    operator=(const inplace_vector_destruct_base&& other) noexcept(std::is_nothrow_move_constructible_v<T> &&
...
struct inplace_vector_base : public inplace_vector_destruct_base<T, Capacity> {
    ...
    inplace_vector_base(const inplace_vector_base& other) noexcept(std::is_nothrow_copy_constructible_v<T>)
        : inplace_vector_destruct_base<T, Capacity>(other.size) {
        std::ranges::copy(other.begin(), other.end(), begin());
    }
    inplace_vector_base(inplace_vector_base&& other) noexcept(Capacity == 0 || std::is_nothrow_move_constructible_v<T>)
        : inplace_vector_destruct_base<T, Capacity>(other.size) {
        std::ranges::copy(other.begin(), other.end(), begin());
    ...
    inplace_vector_base&
    operator=(const inplace_vector_base& other) noexcept(std::is_nothrow_copy_constructible_v<T> &&
                                                         std::is_nothrow_copy_assignable_v<T>) {
    ...
    inplace_vector_base& operator=(inplace_vector_base&& other) noexcept(Capacity == 0 ||
                                                                         (std::is_nothrow_move_constructible_v<T> &&
                                                                          std::is_nothrow_move_assignable_v<T>)) {
        const auto diff = static_cast<std::ptrdiff_t>(other.size() - size());
        // other size is less than size
    ...
    constexpr inplace_vector_base(const size_type size) : inplace_vector_destruct_base<T, Capacity>(size) {}

    // [containers.sequences.inplace.vector.members] size/capacity
    constexpr void      change_size(const std::ptrdiff_t count) noexcept { this->size_ += count; }
    constexpr size_type size() const noexcept { return static_cast<size_type>(this->size_); }
    constexpr bool      empty() const noexcept { return this->size_ == 0; }

    // [containers.sequences.inplace.vector.data], data access
    constexpr T*       data() noexcept { return std::launder(reinterpret_cast<T*>(this->elems)); }
    constexpr const T* data() const noexcept { return std::launder(reinterpret_cast<const T*>(this->elems)); }
    ...
    const_iterator begin() const noexcept { return std::launder(reinterpret_cast<const T*>(this->elems)); }

    iterator end() noexcept { return std::launder(reinterpret_cast<T*>(this->elems) + this->size()); }
    ...
    static constexpr void uninitialized_value_construct(iterator first, iterator last) noexcept {
        std::fill(first, last, T());
    }

    static constexpr void uninitialized_fill(iterator first, iterator last, const T& value) noexcept {
        std::fill(first, last, value);
    }

    template <typename Iter>
    static constexpr void uninitialized_copy(Iter first, Iter last, iterator dest) noexcept {
...
class inplace_vector : public inplace_vector_base<T, Capacity> {
  ...
  public:
    ...
    constexpr inplace_vector(InputIterator first, InputIterator last) : base(std::distance(first, last)) {
        if (first != last) {
    ...
    constexpr inplace_vector& operator=(std::initializer_list<T> il) {
        ...
        if (diff < 0) {
            ...
            const iterator new_end = std::ranges::copy(il.begin(), il.end(), this->begin());
    ...
    constexpr reference operator[](const size_type count) noexcept { return *(this->begin() + count); }

    constexpr const_reference operator[](const size_type count) const noexcept { return *(this->begin() + count); }
    ...
    constexpr pointer try_push_back(const T& x) noexcept(std::is_nothrow_copy_constructible_v<T>) {
        if (this->size() == Capacity) {
            return nullptr;
        }
    ...
    constexpr pointer try_push_back(T&& x) noexcept(std::is_nothrow_move_constructible_v<T>) {
        if (this->size() == Capacity) {
            return nullptr;
        }
    ...
    */
    ...
    constexpr reference unchecked_push_back(const T& x) noexcept(std::is_nothrow_constructible_v<T>) {
        return this->unchecked_emplace_back(x);
    };
    constexpr reference unchecked_push_back(T&& x) { return this->unchecked_emplace_back(std::move(x)); };
    ...
    template <class InputIterator>
    constexpr inplace_vector::iterator insert(const_iterator position, InputIterator first, InputIterator last) {
        const iterator pos   = position;
        const iterator end   = this->end();
    ...
    constexpr iterator insert(const_iterator position, std::initializer_list<T> il) {
        ...
        if (this->size() + count < Capacity) {
            ...
            throw std::out_of_range("inplace_vector::insert(const_iterator, initializer_list)");
        }
        if (count == 0) {
            return pos;
        }
    ...
    constexpr void swap(inplace_vector& x) noexcept(Capacity == 0 || (std::is_nothrow_swappable_v<T> &&
                                                                      std::is_nothrow_move_constructible_v<T>)) {
        if (this->size() < x.size()) {
    ...
    operator==(const inplace_vector& x,
               const inplace_vector& y) noexcept(noexcept(std::equal(x.begin(), x.end(), y.begin(), y.end()))) {
        return std::equal(x.begin(), x.end(), y.begin(), y.end());
    }

    constexpr friend std::compare_three_way_result<T> operator<=>(const inplace_vector& x, const inplace_vector& y) {
        return std::lexicographical_compare(x.begin(), x.end(), y.begin(), y.end());
    };
    constexpr friend void swap(inplace_vector& x, inplace_vector& y) noexcept(
        Capacity == 0 || (std::is_nothrow_swappable_v<T> && std::is_nothrow_move_constructible_v<T>)) {
        x.swap(y);
    }
};
} // namespace beman::inplace_vector
