...
struct inplace_vector_base : public inplace_vector_destruct_base<T, Capacity> {
  ...
  static constexpr void uninitialized_value_construct(iterator first,
                                                      iterator last) noexcept {
    std::fill(first, last, T());
  }

  static constexpr void uninitialized_fill(iterator first, iterator last,
  ...
  */
  ...
  constexpr iterator emplace(const_iterator position, Args &&...args) {
    ...
    if (this->size() == Capacity) {
      ...
      throw std::out_of_range(
          "inplace_vector::emplace(const_iterator, Args...)");
    }
    if (pos == end) {
      this->unchecked_emplace_back(std::forward<Args>(args)...);
    } else {
  ...
  constexpr iterator insert(const_iterator position, size_type n, const T &x) {
    ...
    if (this->size() + n < Capacity) {
      ...
      throw std::out_of_range(
          "inplace_vector::insert(const_iterator, size_type, T)");
    }
    if (n == 0) {
      return pos;
    }
