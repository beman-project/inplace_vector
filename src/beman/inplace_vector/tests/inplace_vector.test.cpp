...
constexpr void test() {
    ...
    static_assert(std::is_same<decltype(bracket_assign), typename vec::reference>::value, "");
    assert(bracket_assign == T(4));

    // Continue with const reference.
    auto&& const_bracket = const_range[3];
    static_assert(std::is_same<decltype(const_bracket), typename vec::const_reference>::value, "");
    ...
    static_assert(std::is_same<decltype(const_front), typename vec::const_reference>::value, "");
    ...
    static_assert(std::is_same<decltype(const_back), typename vec::const_reference>::value, "");
    ...
    static_assert(std::is_same<decltype(const_data), typename vec::const_pointer>::value, "");
